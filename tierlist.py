...
async def tier_remove(interaction: discord.Interaction, kullanıcı: discord.Member, rol: discord.Role):
    ...
    if gid not in data or rid not in data[gid] or str(kullanıcı.id) not in data[gid][rid]:
        return await interaction.response.send_message(f"{kullanıcı.mention} `{rol.name}` tier'ında bulunamadı.", ephemeral=True)
    ...
    if rol in kullanıcı.roles:
        ...
        mesaj = f"`{rol.name}` kullanıcıda yoktu."
...
async def tier_show(interaction: discord.Interaction):
    ...
    for rid, uids in data[gid].items():
        ...
        mentions = [interaction.guild.get_member(int(u)).mention for u in uids if interaction.guild.get_member(int(u))]
...
@app_commands.describe(
    player_name="Başlıkta gözükecek oyuncu adı", kit="Test edilecek kit adı",
    user="Oyuncu (etiketle)", tester="Tester (etiketle)",
    in_game_name="Oyun içi isim", new_tier="Yeni Tier (rol etiketle)",
    old_tier="Eski Tier (rol etiketle)", score="Maç skoru (örn. 2-1)",
    server="Sunucu adı", winner="Kazanan (etiketle)", channel="Raporun gideceği kanal"
)
async def tierver(
    interaction: discord.Interaction, player_name: str, kit: str,
    user: discord.Member, tester: discord.Member, in_game_name: str,
    new_tier: discord.Role, old_tier: discord.Role,
    score: str, server: str, winner: discord.Member, channel: discord.TextChannel
):
    if not any(r.name.lower() == "tester" for r in interaction.user.roles):
        return await interaction.response.send_message("❌ Bu komutu sadece Tester rolü kullanabilir.", ephemeral=True)
    # veritabanı
...
class TicketPanel(discord.ui.View):
    ...
    async def _create_ticket(self, interaction: discord.Interaction, kit_name: str):
        ...
        # Burada Tester rolüne de erişim veriyoruz
        if tester_role:
    ...
    async def smp(self, interaction, button): await self._create_ticket(interaction, "Smp")

    @discord.ui.button(label="Beast", style=discord.ButtonStyle.blurple, custom_id="ticket_beast")
    async def beast(self, interaction, button): await self._create_ticket(interaction, "Beast")

    @discord.ui.button(label="Nethpot", style=discord.ButtonStyle.red, custom_id="ticket_nethpot")
    async def nethpot(self, interaction, button): await self._create_ticket(interaction, "Nethpot")

    @discord.ui.button(label="Gapple", style=discord.ButtonStyle.green, custom_id="ticket_gapple")
    async def gapple(self, interaction, button): await self._create_ticket(interaction, "Gapple")

    @discord.ui.button(label="Diapot", style=discord.ButtonStyle.blurple, custom_id="ticket_diapot")
    async def diapot(self, interaction, button): await self._create_ticket(interaction, "Diapot")

    @discord.ui.button(label="Crystal", style=discord.ButtonStyle.blurple, custom_id="ticket_crystal")
    async def crystal(self, interaction, button): await self._create_ticket(interaction, "Crystal")

    @discord.ui.button(label="Axe", style=discord.ButtonStyle.red, custom_id="ticket_axe")
    async def axe(self, interaction, button): await self._create_ticket(interaction, "Axe")

    @discord.ui.button(label="Elytra", style=discord.ButtonStyle.green, custom_id="ticket_elytra")
    async def elytra(self, interaction, button): await self._create_ticket(interaction, "Elytra")
